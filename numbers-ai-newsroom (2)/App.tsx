...
import { 
  ...
  Github, ExternalLink, Clock, ShieldAlert
} from 'lucide-react';
...
export default function App() {
  ...
  const handleLogin = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({ 
        provider: 'google', 
        options: { 
          redirectTo: window.location.origin, // 동적으로 현재 도메인 사용
      ...
      console.error(e);
      alert(`로그인 오류: ${e.message}. 구글 OAuth 설정을 다시 확인해 주세요.`);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-zinc-950 p-6">
        <div className="w-full max-w-md bg-white dark:bg-zinc-900 rounded-[40px] p-10 lg:p-16 shadow-2xl text-center space-y-10 animate-fade-in relative overflow-hidden">
          ...
          <p className="text-[10px] text-gray-400 font-medium leading-relaxed">블로터 및 넘버스 소속 기자만 사용 가능합니다.<br/>최초 로그인 시 관리자 승인이 필요합니다.</p>
        </div>
      </div>
    );
  }
  ...
  if (userProfile.status === UserStatus.PENDING) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-zinc-950 p-6">
        <div className="w-full max-w-md bg-white dark:bg-zinc-900 rounded-[40px] p-10 lg:p-16 shadow-2xl text-center space-y-8 animate-fade-in">
          <div className="w-20 h-20 bg-yellow-50 dark:bg-yellow-950/20 rounded-3xl flex items-center justify-center mx-auto text-yellow-500">
            <Clock size={48}/>
          </div>
          <div className="space-y-3">
            <h2 className="text-2xl font-black dark:text-white">승인 대기 중</h2>
            <p className="text-gray-500 dark:text-gray-400 text-sm leading-relaxed">
              안녕하세요, <strong>{userProfile.name}</strong>님.<br/>
              현재 시스템 관리자의 승인을 기다리고 있습니다.<br/>
              잠시 후 다시 접속해 주세요.
            </p>
          </div>
          <div className="pt-6 border-t dark:border-zinc-800 flex flex-col gap-4">
            <p className="text-[10px] text-zinc-400 font-bold">관리자: {ADMIN_NAME} ({ADMIN_EMAIL})</p>
            <button onClick={handleLogout} className="text-sm font-bold text-gray-400 hover:text-zinc-600 dark:hover:text-zinc-200">다른 계정으로 로그인</button>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-10 no-scrollbar">
          ...
          {tab === 'admin' && <div className="p-10 text-center space-y-8 animate-fade-in">
            ...
            <div className="max-w-md mx-auto p-6 bg-white dark:bg-zinc-900 rounded-3xl border dark:border-zinc-800 shadow-lg space-y-4">
              ...
              <a href={REPO_URL} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-4 bg-gray-50 dark:bg-zinc-800 rounded-2xl border dark:border-zinc-700 hover:border-blue-500 transition-all group">
